...
void Graph::readGraph(std::vector<std::vector<int>>& graph, std::vector<int>& weight) {
	this->n = graph.size();
	head = new Vertex[n];
	
	for (int i = 0; i < graph.size();i++) {
		std::vector<int>& adj_list = graph[i];
		for (auto& neighbor : adj_list) {
			head[i].adjacent.push_back(neighbor);
		}
		/*printf("%d ", i);
		for (auto& neighbor : adj_list) {
			printf("%d ", neighbor);
	...
	//clock_t start = clock();
	//printf("Graph: %s\n", dir);
	//FILE* f = open_file((dir + "\\" + dir + ".deg").c_str(), "rb");
	...
	//f = open_file((dir + "\\" + dir + ".gra").c_str(), "rb");
	...
	//		head[i].adjacent_state.push_back(1);
...
/*
	reconstruct graph for maintaining edge states.
*/
...
void Graph::bfsReduction(VI& pone, VI& degree_one, VI& degree_two, bool* adj) {
	// count sorting;
	int MAX_WEIGHT = 0;
	for(int i = 0;i < n;i++) if (!head[i].del && head[i].neiSize > 0) { 
	...
	for(int i = 0; i < n; i++) if(!head[i].del && head[i].neiSize > 0){ count[head[i].weight]++; }
	for (int i = 1; i < MAX_WEIGHT; i++) { count[i] += count[i - 1]; }
	...
	for (int k = survival; k > 0; k--){
		...
		for(auto n: head[i].adjacent) if(!head[n].del){
			...
			while(!Q.empty()){
				int h = Q.front();
				Q.pop();
				for (auto nn : head[h].adjacent) if (adj[nn] && !visited[nn]){
					Q.push(nn);
				}
				if (!visited[h]) {
					bool ok = 0;
...
void Graph::htNeighborhood() {
	...
	std::vector<int> pone, dominated, degree_two, degree_one;
	std::vector<pair<int, int> > backTrack;
	...
	for (int i = 0; i < n; i++) {
		if (head[i].del == 0 && head[i].neiSize > 0) {
			...
			for (int j = 0; j < index; j++) { head[i].adjacent[j] = neis[j]; }
			delete[] neis;
		}
	}
	...
	for (int k = 0; k < n; k++) {
		if (head[k].del == 0 && head[k].neiSize > 0) {
			if (head[k].neiSum - head[k].weight > max_d) { max_d = head[k].neiSum - head[k].weight; }
	...
	for (int k = backTrack.size() - 1; k >= 0; k--) {
		...
		if (a == n) {
			...
			// circle ends with two adjacent vertex.
				...
				else { printf("error info: T[0] and T[0] cann't exist at the same time\n"); }
...
//	printf("total weight :%d ,  remainder : %d , total time cost : %f\n", total_weight, remainder, (float)(end - start) * 1000 / CLOCKS_PER_SEC);
	delete[] weight_copy;
}
...
void Graph::htPvertex() {
	...
	bfsReduction(pone, degree_one, degree_two, adj);
	int count1 = 0, d_two1 = 0;
	for (int k = 0; k < n; k++) {
		if (head[k].del == 0 && head[k].neiSize > 0) {
			if (head[k].neiSum - head[k].weight > max_d) { max_d = head[k].neiSum - head[k].weight; }
			if (head[k].neiSize == 2) d_two1++;
			count1++;
		}
	}
//	printf("degree_two_size: %d \nKernel Size:%d\n", d_two1, n - count1);
	...
	for (int i = vs.size() - 1; i >= 0; i--) {
		...
		for (auto n : head[u].adjacent) if (head[n].del == 0) { adj[n] = 1; }
		adj[u] = 1;

		int dominate_sum = 0, edge_sum = 0;
		bool dominate = 0, two_hop_key = 0;
		std::vector<int> two_hop;
		for (auto x : head[u].adjacent) if (!head[x].del /*&& head[x].neiSize <= head[u].neiSize*/) {
			...
			for (auto nn : head[x].adjacent) if (!head[nn].del) {
				if (!adj[nn]) {
					...
					if (head[nn].weight + head[u].weight >= head[nn].neiSum + head[u].neiSum - AdjShare[nn]) {
						//printf("run");
						for (auto nei : head[u].adjacent) {
							deleteVertex(nei, u, pone, degree_one, degree_two);
						}
						for (auto nei : head[nn].adjacent) {
							deleteVertex(nei, nn, pone, degree_one, degree_two);
						}
						two_hop_key = 1;
						break;
					}
					...
					if (nn != u) { nei_sum_of_x += head[nn].weight; }
					++tri_count;
				}
			}
		...
		for (auto n : head[u].adjacent) if (head[n].del == 0) {
			adj[n] = 0;
			times[n] = 0;
		}
		adj[u] = 0;
	...
	for (int i = 0; i < n; i++) {
		if (head[i].del == 0 && head[i].neiSize > 0) {
			...
			for (auto n : head[i].adjacent) {
				if (head[n].del == 0) {
					if ((head[i].weight >= head[i].neiSum - head[n].weight) || (head[n].weight >= head[n].neiSum - head[i].weight)) {
						kk++;
					}
				}
			}
...
void Graph::clearPone(bool* adj, VI& pone, VI& degree_one, VI& degree_two, vector<pair<int, int>>& backTrack,vector<vector<int>>& circles,bool key) {
	while (!pone.empty() || !degree_one.empty() || !degree_two.empty()) {
		while (!pone.empty() || !degree_one.empty()) {
			...
			while (!degree_one.empty() && pone.empty()) {
				...
				degree_one.pop_back();
				if (head[v].neiSize != 1 || head[v].del ) { continue; }
				int neibor = -1;
				...
				if (head[neibor].weight > head[v].weight) {
					...
					for (auto n : head[neibor].adjacent) if (head[n].del == 0) {
						head[n].neiSum -= head[v].weight;
						if (head[n].neiSum <= head[n].weight) { pone.push_back(n); }
		...
		while (pone.empty() && degree_one.empty() && !degree_two.empty()) {
			...
			if(isNei(n1,n2)){ 	
				...
				deleteVertex(v, pone, degree_one, degree_two);
				for (auto n : head[n1].adjacent) if (head[n].del == 0) {
					head[n].neiSum -= head[v].weight;
					if (head[n].neiSum <= head[n].weight) { pone.push_back(n); }
				}
				...
				deleteVertex(v, pone, degree_one, degree_two);
				for (auto n : head[n2].adjacent) if (head[n].del == 0) {
					head[n].neiSum -= head[v].weight;
					if (head[n].neiSum <= head[n].weight) { pone.push_back(n); }
				}
			...
			// if n1,n2 is not adjacent;
				...
				if (key) {    // edges are not allowed to be inserted.
					...
					if (head[v].weight >= min(head[n1].weight, head[n2].weight)) {
						...
						for (auto n : head[smaller].adjacent) if (!head[n].del) {
							adj[n] = 1;
						}
						for (auto n : head[bigger].adjacent) if (!head[n].del && !adj[n]) {
						...
						if (head[v].weight >= max(head[n1].weight, head[n2].weight)) {
							...
							for (auto nei : head[smaller].adjacent) if (!head[nei].del) {
							...
							for (auto nei : head[bigger].adjacent) if (!head[nei].del) {
			...
			if (next == v) {
				...
				int index = 0;
				for (int j = 1; j < deq_size; j++) { dp[index++] = head[deq[j]].weight; }						
				...
				if (dp_sum2 > dp_sum1 + head[v].weight) {
					...
					for (auto n : head[v].adjacent) if(!head[n].del){
						deleteVertex(n, pone, degree_one, degree_two);
					}
				}
				delete[] dp;
			...
			if (next == final2) {
				...
				int delta = 0;
				if (deq_size < 3) { printf("error code:101"); }
				int* dp = new int[deq_size + 1];
				for (int j = 0; j < deq_size; j++) { dp[j] = head[deq[j]].weight; }
				int dp_sum1 = 0, dp_sum2 = 0;
				...
				if (head[next].weight > delta) {
					...
					for (auto n : head[next].adjacent) {
						if (head[n].del == 0) {
							head[n].neiSum -= delta;
							if (head[n].neiSum <= head[n].weight && n != deq[0] && n != deq[deq_size - 1]) {
								pone.push_back(n);
							}
						}
					}
			...
			//// path reduction #3 the non-degree-two vertices are adjacent.
				...
				if (dp_sum >= head[next].weight + dp_b) {
					...
					head[next].weight -= delta;
					for (auto n : head[next].adjacent) if (head[n].del == 0) {
						head[n].neiSum -= delta;
						if (head[n].neiSum <= head[n].weight && n != deq[0] && n != deq[deq_size - 1]) { pone.push_back(n); }
					}

					delta = dp_sum - dp_f;
					head[final2].weight -= delta;
					for (auto n : head[final2].adjacent) if (head[n].del == 0) {
						head[n].neiSum -= delta;
						if (head[n].neiSum <= head[n].weight && n != deq[0] && n != deq[deq_size - 1]) { pone.push_back(n); }
					}
...
void Graph::clearPone(VI& pone, VI& degree_one, VI& degree_two, vector<pair<int, int>>& backTrack, vector<vector<int>>& circles, VI& dominated, int* edges, int* tri, bool* dominate, bool* adj) {
	while (!dominated.empty() || !pone.empty() || !degree_one.empty() || !degree_two.empty()) {
		while (!dominated.empty() || !pone.empty() || !degree_one.empty()) {
			while (!dominated.empty() || !pone.empty()) {
				...
				while (dominated.empty() && !pone.empty()) {
					int u = pone.back();
					pone.pop_back();
					if (head[u].del || head[u].neiSize < 1 || head[u].weight < head[u].neiSum) continue;
					for (auto n : head[u].adjacent) {
						deleteVertex(n, tri, edges, adj, dominate, pone, degree_one, degree_two, dominated);
					}
				}
			}
			while (dominated.empty() && pone.empty() && !degree_one.empty()) {
				...
				if (head[neibor].weight > head[u].weight) {
					...
					deleteVertex(u, pone, degree_one, degree_two); // no triangle destroyed. but neibor might be dominated.
					...
					if (!dominate[neibor]) {
						for (auto n : head[neibor].adjacent) if (!head[n].del) { adj[n] = 1; }
						VI& Nei_adj = head[neibor].adjacent;
						for (int i = 0, Nei_size = Nei_adj.size(); i < Nei_size; i++) {
							int v = Nei_adj[i];
							if (head[v].del || head[v].weight < head[neibor].weight || head[v].neiSize > head[neibor].neiSize) continue;
							if (tri[edge_id(neibor, i)] + 1 == head[v].neiSize) {
								dominate[neibor] = 1;
								dominated.push_back(neibor);
							}
						}
						for (auto n : head[neibor].adjacent) if (!head[n].del) { adj[n] = 0; }
					}
				}
			}
		}
		...
		while (dominated.empty() && pone.empty() && degree_one.empty() && !degree_two.empty()) {
			...
			if (isNei(n1, n2)) {
				bool ok = 0;
				if (head[n1].weight <= head[v].weight) {
					deleteVertex(n1, tri, edges, adj, dominate, pone, degree_one, degree_two, dominated);
					continue;
				}
				if (head[n2].weight <= head[v].weight) {
					deleteVertex(n2, tri, edges, adj, dominate, pone, degree_one, degree_two, dominated);
					continue;
				}
				head[n1].weight -= head[v].weight;
				...
				if (!dominate[n1]) {
					for (auto n : head[n1].adjacent) if (!head[n].del) { adj[n] = 1; }
					VI& Nei_adj = head[n1].adjacent;
					for (int i = 0, Nei_size = Nei_adj.size(); i < Nei_size; i++) {
						int v = Nei_adj[i];
						if (head[v].del || head[v].weight < head[n1].weight || head[v].neiSize > head[n1].neiSize) continue;
						if (tri[edge_id(n1, i)] + 1 == head[v].neiSize) {
							dominate[n1] = 1;
							dominated.push_back(n1);
						}
					}
					for (auto n : head[n1].adjacent) if (!head[n].del) { adj[n] = 0; }
				}
				...
				if (!dominate[n2]) {
					for (auto n : head[n2].adjacent) if (!head[n].del) { adj[n] = 1; }
					VI& Nei_adj = head[n2].adjacent;
					for (int i = 0, Nei_size = Nei_adj.size(); i < Nei_size; i++) {
						int v = Nei_adj[i];
						if (head[v].del || head[v].weight < head[n2].weight || head[v].neiSize > head[n2].neiSize) continue;
						if (tri[edge_id(n2, i)] + 1 == head[v].neiSize) {
							dominate[n2] = 1;
							dominated.push_back(n2);
						}
					}
					for (auto n : head[n2].adjacent) if (!head[n].del) { adj[n] = 0; }
				}
				backTrack.push_back(make_pair(v, n + 1));
				continue;
			}
			...
			if (next == final2) {
				...
				if (head[next].weight > delta) {
					...
					deleteVertex(next, tri, edges, adj, dominate, pone, degree_one, degree_two, dominated);
				}
			}
			//// path reduction #3 the non-degree-two vertices are adjacent.
			else if (isNei(next, final2)) {
				...
				if (dp_sum >= head[next].weight + dp_b) {
					...
					deleteVertex(final2, tri, edges, adj, dominate, pone, degree_one, degree_two, dominated);
				}
				else {
					backTrack.push_back(make_pair(n, 2));
					circles.push_back(vector<int>());
...
void Graph::findTwoNeibor(int v, int& n1, int& n2) {
	...
	if (error != 2) { printf("vertex %d error neibor: %d != 2\n", v , error); }
}
...
bool Graph::isNei(int v1, int v2) {
	...
	for (auto n : *target) if (!head[n].del) {
		if (n == id) return 1;
	}
	return 0;
}

void Graph::neighborhood_reduction(VI& pone, VI& degree_one, VI& degree_two, vector<pair<int, int>>& backTrack, vector<vector<int>>& circles, bool* adj) {
	for (int i = 0; i < n; i++) if (!head[i].del) {
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
			adj[nei] = 1;
		}
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
			int w = 0;
			for (auto nn : head[nei].adjacent) if (!head[nn].del && adj[nn]) {
				w += head[nn].weight;
...
void Graph::symmetric_folding(VI& pone, VI& degree_one, VI& degree_two, vector<pair<int, int>>& backTrack, vector<vector<int>>& circles, bool* adj, int* AdjShare) {
	for (int i = 0; i < n; i++) if (!head[i].del) {
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
			adj[nei] = 1;
		}
		...
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
			for (auto nn : head[nei].adjacent) if (!head[nn].del && !adj[nn]) {
				...
				two_hop.push_back(nn);
				if (head[i].weight >= head[i].neiSum - AdjShare[nn] && head[nn].weight >= head[nn].neiSum - AdjShare[nn]) {
					// fold nn and i
					...
					for (auto k : head[i].adjacent) if (!head[k].del) {
						if (adj[k]) {
							head[k].neiSum += head[nn].weight - head[i].weight;
							head[k].neiSize++;
							head[k].adjacent.push_back(nn);
...
void Graph::common_neighbor_reduction(VI& pone, VI& degree_one, VI& degree_two, vector<pair<int, int>>& backTrack, vector<vector<int>>& circles, bool* adj) {
	//
	int times = 0;
	for (int i = 0; i < n; i++) if(!head[i].del){
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
		...
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
			if ((head[i].weight >= head[i].neiSum - head[nei].weight) || (head[nei].weight >= head[nei].neiSum - head[i].weight)) {
				times++;
				for (auto nn : head[nei].adjacent) if (!head[nn].del && adj[nn]) {
					deleteVertex(nn, pone, degree_one, degree_two);	
				}
...
void Graph::initial_reduction(VI& pone, VI& degree_one, VI& degree_two, vector<pair<int, int>>& backTrack, vector<vector<int>>& circles,bool* adj, int* AdjShare) {
	VI vs;
	for (int i = 0; i < n; i++) if (!head[i].del && head[i].neiSize > 0) {
		vs.push_back(i);
	}
	...
	for (int i = vs.size() - 1; i >= 0; i--) {
		...
		bool dominate = 0,two_hop_key = 0;
		std::vector<int> two_hop;
		for (auto x : head[u].adjacent) if (!head[x].del) {
			...
			for (auto nn : head[x].adjacent) if (!head[nn].del) {
				if (!adj[nn]) {
					...
					// check if nn and u are symmetry.
					if (head[u].neiSum - AdjShare[nn] <= head[u].weight && head[nn].neiSum - AdjShare[nn] <= head[nn].weight) {				
						//printf("symmetric foldig\n");
						// u and nn are bound to be in the MWIS together.
						deleteVertex(u,pone,degree_one,degree_two);
						head[nn].weight += head[u].weight;
						...
						for (auto k : head[u].adjacent) if (!head[k].del) {
							if (adj[k]) {
								head[k].neiSum += head[nn].weight - head[u].weight;
								head[k].neiSize++;
								head[k].adjacent.push_back(nn);
...
void Graph::compute_triangles(int* tri, int* edges, int* AdjShare, bool* adj, bool* dominate,VI& pone, VI& degree_one, VI& degree_two, VI& dominated, vector<pair<int, int>>& backTrack, vector<vector<int>>& circles) {
		...
		if (dominate[u]) {
			...
			for (int k = 0, U_size = U_adj_list.size(); k < U_size; k++) {
				...
				if (!head[v].del && order[v] > i) {
					...
					for (int j = 0, N_size = N_adj_list.size(); j < N_size; j++) if (!head[N_adj_list[j]].del) {
						int w = N_adj_list[j];
						int eid = edge_id(v, j);
						if (adj[w]) {
							tri[eid]--;
						}
						if (!dominate[v] && tri[eid] + 1== head[w].neiSize && head[v].weight <= head[w].weight) {
							dominate[v] = 1;
							dominated.push_back(v);
						}
						if (!dominate[w] && tri[eid] + 1 == head[v].neiSize && head[w].weight <= head[v].weight) {
							dominate[w] = 1;
			...
			for (int j = 0, U_size = U_adj_list.size(); j < U_size; j++) {
				...
				if (!dominate[v] && tri[eid] + 1 == head[u].neiSize && head[v].weight <= head[u].weight) {
					dominate[v] = 1;
...
void Graph::deleteVertex(int u, int* tri,int* edges, bool* adj,bool* dominate, VI& pone, VI& degree_one, VI& degree_two, VI& dominated) {
...
void Graph::edge_reduction(bool* adj) {
	/*
		reduce edge
	*/
	for (int i = 0; i < n; i++) if (!head[i].del) {
		...
		for (auto nei : head[i].adjacent) if (!head[nei].del) {
			int w = 0;
			for (auto nn : head[nei].adjacent) if (!head[nn].del && /*attention to whether adjcent or not*/!adj[nn] ) {
				w += head[nn].weight;
				if (head[i].weight >= head[i].neiSum - w) {
					//deleteVertex(nei, pone, degree_one, degree_two);
					break;
...
void Graph::htThreeAll(vector<int>& mvc, vector<int>& mis) {
	//clear the invalid vertice��
	for (int i = 0; i < n; i++) {
		//clear the useless vertex
		if (head[i].weight <= 0) {
...
void Graph::dtThreeAll() {
	...
	for (int k = 0; k < n; k++) {
		if (head[k].del == 0 && head[k].neiSize > 0) {
			if (head[k].neiSize > max_d) { max_d = head[k].neiSize; }
			if (head[k].neiSize == 2) d_two++;
			count++;
		}
	}
...
void Graph::wtThreeAll() {
	...
	for (int k = 0; k < n; k++) {
		if (head[k].del == 0 && head[k].neiSize > 0) {
			if (head[k].weight > max_d) { max_d = head[k].weight; }
			if (head[k].neiSize == 2) d_two++;
			count++;
		}
	}
...
void Graph::path_recover(int* V, int num) {
	...
	for (int i = 2; i < num; i++) {
		head[V[i]].weight + dp[i - 2] >= dp[i - 1] ? (track[i] = 1,dp[i] = head[V[i]].weight + dp[i - 2] ) : (track[i] = 0, dp[i] = dp[i-1]);
	}
	for (int i = num - 1; i >= 0;) {
		if (track[i]) {
			head[V[i]].del = 0;
...
int Graph::edgeHandler(vector<int>& list, int len) {
	switch (len) {
		case 2: return oneEdge(list); 
		case 4: return twoEdge(list); 
		case 6: return threeEdge(list); 
	}
	return 0;
}


int Graph::oneEdge(vector<int>& list) {
...
int Graph::threeEdge(vector<int>& list) {
	...
	if (index == 3) {
		return max(max(head[list[0]].weight, head[list[1]].weight), max(head[list[2]].weight, head[list[3]].weight));
	}
	VI one, two, three;
	for (int i = 0; i < index; i++) {
		switch (deg[i]) {
			case 1:one.push_back(m2[i]); break;
			case 2:two.push_back(m2[i]); break;
			case 3:three.push_back(m2[i]); break;
		}
	}
	if (index == 5) {
		int max1 = 0, max2 = 0;
...
void Graph::htDegreeOne(){
	...
	// ("total weight :%d ,  remainder : %d , total time cost : %f\n", total_weight, remainder, (float)(end - start) * 1000 / CLOCKS_PER_SEC);
	delete[] weight_copy;
}
